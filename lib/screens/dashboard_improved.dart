  ...
  Future<void> _loadUserData() async {
    try {
      ...
      if (userInfo.containsKey('error') || userInfo['user'] == null) {
        // Si hay error cargando el perfil, limpiar datos y redirigir al login
      ...
      // Error cargando datos del usuario - redirigir al login
      await FeelinPayService.logout();
      if (mounted) {
        Navigator.pushReplacementNamed(context, '/login');
      }
  ...
  Future<void> _llenarDatosPrueba() async {
    try {
      ...
      if (result['success']) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(result['error'] ?? 'Error llenando datos de prueba'),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Future<void> _procesarPagoPrueba() async {
    ...
    try {
      ...
      if (result['success'] && mounted) {
        // Llenar Google Sheets automáticamente con datos de prueba
        final ahora = DateTime.now();
        final datosPago = {
          'pagador': 'Cliente de Prueba',
          'monto': 25.0,
          'codigoSeguridad': '123', // Código de 3 dígitos para Yape
          'fecha': ahora.toIso8601String().split('T')[0], // YYYY-MM-DD
          'hora':
